...
class TBDeviceMqttClientBase:
    ...
    def claim_device(self, secret_key=None, duration_ms=None):
        ...
        print(
            f"Sending claim request to topic '{CLAIMING_TOPIC}' with payload: {payload}"
        )
    ...
    def unsubscribe_from_attribute(self, subscription_id):
        for attribute in self.__device_sub_dict:
            if self.__device_sub_dict[attribute].get(subscription_id):
                ...
                print(
                    "Unsubscribed from {0}, subscription id {1}".format(
        ...
        if topic.startswith(ATTRIBUTES_TOPIC):
            ...
            if (
                ...
                firmware_tail = (
                    ...
                    else self.firmware_info[FW_SIZE_ATTR] % self.__chunk_size
                )
                self.__chunk_count = (
                    0
                    if not self.__chunk_size
                    else int(self.firmware_info[FW_SIZE_ATTR] / self.__chunk_size)
                    + (0 if not firmware_tail else 1)
                )
                self.__get_firmware()
    ...
    def __get_firmware(self):
        ...
        self._client.publish(
            "v2/fw/request/{0}/chunk/{1}".format(
                self.__firmware_request_id, self.__current_chunk
            ),
            payload,
            qos=1,
        )
    ...
    def __process_firmware(self):
        ...
        if verification_result:
            ...
            self._client.publish(
                "v1/devices/me/attributes/request/{0}".format(self.__request_id),
                dumps({"sharedKeys": REQUIRED_SHARED_KEYS}),
            )
            return

    @staticmethod
    def get_provision_request(
